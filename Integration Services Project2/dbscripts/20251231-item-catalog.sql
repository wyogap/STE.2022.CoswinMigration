...
WHERE A.IMAGE IS NOT NULL AND COALESCE(A.ITEMID,0)>0 AND A.ATTACHMENT=0
ORDER BY A.ID;
...
--	SELECT 
--		B.MAX_ID + ROW_NUMBER() OVER (ORDER BY A.ID) AS IMGLIBID
--		, ROW_NUMBER() OVER (PARTITION BY A.ITEMID) RN
--		, 'ITEM' AS REFOBJECT
--		, A.ITEMID AS REFOBJECTID, A.IMAGENAME, A.IMAGE, A.MIMETYPE, A.DUPLICATE, A.FILENAME, A.PATH, A.ATTACHMENT
--	FROM MIGRATION.PATCH_20251231_ITEM_CATALOG A
--	LEFT JOIN (
--		SELECT COALESCE(MAX(IMGLIBID),0) AS MAX_ID FROM MAXIMO.IMGLIB
--	) B ON 1=1
