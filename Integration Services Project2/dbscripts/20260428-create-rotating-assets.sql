...
-- GET WHAT TO CREATE
INSERT INTO "MIGRATION"."PATCH_20260428_ROTATING_ASSETS" (
...
CREATE OR REPLACE PROCEDURE MIGRATION.STE_PATCH_20260428_ROTATING_ASSETS()
LANGUAGE SQL
SPECIFIC STE_PATCH_20260428_ROTATING_ASSETS
BEGIN
    -- version 1.0
	...
	DECLARE cursor1 CURSOR FOR
		SELECT ID, ITEMNUM, TOCREATE FROM MIGRATION.PATCH_20260428_ROTATING_ASSETS WHERE TOCREATE>0 AND LOCATION!='TRANSIT';
	
	DECLARE CONTINUE HANDLER FOR NOT FOUND
		SET v_counter = -1;
	
	-- LOGGING
	DELETE FROM MIGRATION.STE_MIGRATION_DEBUG_LOG WHERE PACKAGE_NAME='PATCH-20260428-ROTATING-ASSETS';
	...
	LOOP
		...
		FROM (
			...
			FROM MIGRATION.PATCH_20260428_ROTATING_ASSETS A
			JOIN (
				SELECT
		...
		VALUES('PATCH-20260428-ROTATING-ASSETS','CREATED',V_ITEMNUM,V_TOCREATE);
...
-- CREATE THE ASSET
CALL MIGRATION.STE_PATCH_20260428_ROTATING_ASSETS();

-- CNT: 11383
-- SELECT COUNT(*) AS CNT FROM MAXIMO.ASSET WHERE STE_MIGRATIONID=20260428 AND STE_MIGRATIONTS=-1;
...
USING (
	...
	FROM MAXIMO.ASSET a 
	WHERE ste_system IS NOT null AND STE_MIGRATIONID=20260428 AND STE_MIGRATIONTS=-1
...
-- CLEAN UP
DELETE FROM maximo.PLUSTASSETALIAS WHERE STE_MIGRATIONID=20260428;
...
SELECT 
	...
	, 20260428 AS ste_migrationid, ADD_HOURS(CURRENT_TIMESTAMP, 8) AS ste_migrationdate
FROM maximo.asset a
JOIN (
	SELECT isnull(max(PLUSTASSETALIASID),0) AS max_id FROM maximo.PLUSTASSETALIAS
) b ON 1=1
WHERE a.STE_MIGRATIONID=20260428 AND a.STE_MIGRATIONTS=-1
ORDER BY a.ASSETUID
;

-- CLEAN UP
DELETE FROM maximo.PLUSTASSETSTHIST WHERE STE_MIGRATIONID=20260428;
...
-- CLEAN UP
DELETE FROM maximo.ASSETANCESTOR WHERE STE_MIGRATIONID=20260428 AND STE_MIGRATIONTS=-1;
...
-- CLEAN UP
DELETE FROM maximo.ASSETTRANS WHERE STE_MIGRATIONID=20260428 AND STE_MIGRATIONTS=-1;
...
CALL MIGRATION.STE_FINISH_PATCH('20260428_ROTATING_ASSETS');