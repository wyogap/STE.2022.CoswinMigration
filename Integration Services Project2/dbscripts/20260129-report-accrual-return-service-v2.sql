CALL MIGRATION.STE_START_PATCH('20260129-REPORT-ACCRUAL-RETURN-SERVICE');
...
FROM (
	...
	FROM (
		...
		WHERE m.ISSUETYPE='RECEIPT' AND m.STATUS='COMP' AND m.FROMSTORELOC IS NULL AND m.TOSTORELOC!='L100000019292'	
			...
			AND m.LINECOST>0
			-- LEGACY FROM COSWIN: receiptqty != rtnquantity
			AND ( coalesce(m.STE_CSWNRCTQTY,1) <> coalesce(m.STE_CSWNRTNTOSUPL,0) )  
			-- LEGACY FROM COSWIN: receiptstatus != fully invoiced (51)
		...
		SELECT r.matrectransid, r.ponum, r.polinenum, r.porevisionnum, r.transdate, m.transdate AS completeddate, m.actualdate AS actualdate
			, r.itemnum, (m.quantity+COALESCE(rtn.quantity,0)) AS quantity, m.linecost+coalesce(rtn.linecost,0) AS linecost
			, r.STE_CSWNGRNNUM, r.STE_CSWNGRNDATE, r.PACKINGSLIPNUM
        ...
        LEFT JOIN (
        	SELECT rtn.receiptref, rtn.tostoreloc, sum(rtn.quantity) AS quantity, sum(rtn.linecost) AS linecost
		...
		WHERE 1=1
			-- no return or has remaining quantity/linecost
			AND (abs(coalesce(rtn.linecost,0))<m.linecost OR abs(coalesce(rtn.quantity,0)<m.quantity))
			-- ignored invalid receipt
			AND m.LINECOST>0
...
SELECT
	...
	-- REMAINING LINECOST AFTER RETURN
	, m.ACT_LINECOST AS LINECOST
	, 'NS' AS STK_NS
	...
	--, coa.GLCOMP02 AS COSTCENTER 
	--, coa.GLCOMP03 AS SAP_GL_CODE 
	-- REMAINING QUANTITY AFTER RETURN
	, m.ACT_QUANTITY AS QUANTITY
	, m.ORDERQTY
	, l.INVOICEQTY
	, t.ACCEPTEDQTY
	--, m.STATUS
	...
	, m.PL_SAP_GL_CODE
	, m.PONUM, m.POREVISIONNUM, m.POLINENUM
	, m.RTN_QUANTITY, m.RTN_LINECOST
	, m.RECV_QUANTITY, m.RECV_LINECOST
FROM (
	SELECT m.* 
	, CASE WHEN m.GLDEBITACCT IS NULL THEN NULL
	       WHEN length(m.GLDEBITACCT) - length(replace(m.GLDEBITACCT,'-','')) >= 2 THEN
	...
	, COALESCE(rtn.quantity,0) AS RTN_QUANTITY
	, COALESCE(rtn.linecost,0) AS RTN_LINECOST
	, m.linecost AS RECV_LINECOST
	, CASE WHEN m.TRANSDATE<='2025-03-30' THEN COALESCE(m.STE_CSWNACPQTY, m.QUANTITY) 
		   ELSE m.QUANTITY
	  END AS RECV_QUANTITY
	, CASE WHEN m.TRANSDATE<='2025-03-30' THEN COALESCE(m.STE_CSWNACPQTY, m.QUANTITY)+COALESCE(rtn.quantity,0) 
		   ELSE m.QUANTITY+COALESCE(rtn.quantity,0)
	  END AS ACT_QUANTITY
	, m.linecost+COALESCE(rtn.linecost,0) as ACT_LINECOST
	FROM maximo.SERVRECTRANS m
	JOIN maximo.POLINE pl ON pl.PONUM=m.PONUM AND pl.POLINENUM=m.POLINENUM AND pl.REVISIONNUM=m.POREVISIONNUM
	LEFT JOIN maximo.ASSET a ON a.ASSETNUM=m.ASSETNUM
    LEFT JOIN (
    	SELECT rtn.receiptref, sum(rtn.quantity) AS quantity, sum(rtn.linecost) AS linecost 
    	FROM maximo.SERVRECTRANS rtn
    	WHERE rtn.issuetype = 'RETURN'
    	GROUP BY rtn.receiptref
    ) rtn ON 1=1
    	AND rtn.receiptref=m.servrectransid 
	where m.ISSUETYPE IN ('RECEIPT')
		AND m.STATUS='COMP'
		-- no return on service or has remaining quantity/linecost
		AND (m.linecost+COALESCE(rtn.linecost,0)>0.01 
				OR (m.TRANSDATE>'2025-03-30' AND m.quantity+COALESCE(rtn.quantity,0)>0.01)
				OR (m.TRANSDATE<='2025-03-30' AND COALESCE(m.STE_CSWNACPQTY, COALESCE(m.QUANTITY,0))+COALESCE(rtn.quantity,0)>0.01)
		)
		-- LEGACY FROM COSWIN: receiptval > 0
		AND m.LINECOST>0
		-- LEGACY FROM COSWIN: receiptqty != rtnquantity
...
CALL MIGRATION.STE_FINISH_PATCH('20260129-REPORT-ACCRUAL-RETURN-SERVICE');