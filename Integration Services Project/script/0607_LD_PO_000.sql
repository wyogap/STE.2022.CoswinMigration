...
drop procedure if exists ste_0607_LD_PO_pre
GO

CREATE PROCEDURE ste_0607_LD_PO_pre 
	@PackageLogID INT
AS
BEGIN
	-- truncate existing data
	delete from longdescription where STE_MIGRATIONID is not null and ldownertable='PO';
...
drop procedure if exists ste_0607_LD_PO_post
GO

CREATE PROCEDURE ste_0607_LD_PO_post
  @PackageLogID INT
AS
BEGIN
	...
	where STE_MIGRATIONID is not null and ldownertable='PO' AND STE_MIGRATIONSOURCE='REMARK';
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='PO' AND STE_MIGRATIONSOURCE='FOLLOWUP';
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='PO';
