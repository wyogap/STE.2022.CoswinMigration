...
BEGIN
	...
	-- update identity column (route_stopid, not routestopid)
	...
	select @v_cnt=count(STE_MIGRATIONID), @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID) from [ROUTE_STOP];
