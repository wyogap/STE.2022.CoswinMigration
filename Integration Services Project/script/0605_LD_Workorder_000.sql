...
drop procedure if exists ste_0605_LD_Workorder_pre
GO

CREATE PROCEDURE ste_0605_LD_Workorder_pre 
	@PackageLogID INT
AS
BEGIN
	-- truncate existing data
	delete from longdescription where STE_MIGRATIONID is not null and ldownertable='WORKORDER';
...
drop procedure if exists ste_0605_LD_Workorder_post
GO

CREATE PROCEDURE ste_0605_LD_Workorder_post
  @PackageLogID INT
AS
BEGIN
	...
	where STE_MIGRATIONID is not null and ldownertable='WORKORDER' AND STE_MIGRATIONSOURCE='WIP_WO';
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='WORKORDER' AND STE_MIGRATIONSOURCE='WIP_WA';
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='WORKORDER';
