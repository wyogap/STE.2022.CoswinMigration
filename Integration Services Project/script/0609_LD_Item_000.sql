...
drop procedure if exists ste_0609_LD_Item_pre
GO

CREATE PROCEDURE ste_0609_LD_Item_pre 
	@PackageLogID INT
AS
BEGIN
	-- truncate existing data
	delete from longdescription where STE_MIGRATIONID is not null and ldownertable='ITEM';
...
drop procedure if exists ste_0609_LD_Item_post
GO

CREATE PROCEDURE ste_0609_LD_Item_post
  @PackageLogID INT
AS
BEGIN
	...
	where STE_MIGRATIONID is not null and ldownertable='ITEM'
		and STE_MIGRATIONSOURCE='NSITEM-RMK';
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='ITEM'
		and STE_MIGRATIONSOURCE='NSITEM-LD';
