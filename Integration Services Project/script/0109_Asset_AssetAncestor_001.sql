...
-- Add custom columns
-- ------------------
IF COLUMNPROPERTY(OBJECT_ID('dbo.assetancestor'), 'STE_MIGRATIONEQLEVL', 'ColumnId') is null
ALTER TABLE assetancestor
ADD STE_MIGRATIONEQLEVL INT NULL
;
...
BEGIN
	...
	select @v_cnt=count(STE_MIGRATIONID) from assetancestor
	where STE_MIGRATIONID is not null AND hierarchylevels=0;
