...
BEGIN
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from COMPMASTER
	where STE_MIGRATIONID is not null AND STE_MIGRATIONSOURCE = 'DIR_SUBC';
...
BEGIN
	...
	values (
		@PackageName
		, @PackageLogID
		, 'COMPCONTACTMSTR-SUPPLIER_ (CONTACT)'
		, 'COMPLETED'
	...
	values (
		@PackageName
		, @PackageLogID
		, 'COMPCONTACTMSTR-SUPPLIER_ (TECH)'
		, 'COMPLETED'
		, CONCAT('COUNT: ', COALESCE(@v_cnt,0), ', START_ID: ', @v_start_id, ', END_ID: ', @v_end_id)
	);
	...
	values (
		@PackageName
		, @PackageLogID
		, 'COMPCONTACTMSTR-DIR_SUPP (CONTACT)'
		, 'COMPLETED'
		, CONCAT('COUNT: ', COALESCE(@v_cnt,0), ', START_ID: ', @v_start_id, ', END_ID: ', @v_end_id)
	);
	...
	values (
		@PackageName
		, @PackageLogID
		, 'COMPCONTACTMSTR-DIR_MANF (CONTACT)'
		, 'COMPLETED'
		, CONCAT('COUNT: ', COALESCE(@v_cnt,0), ', START_ID: ', @v_start_id, ', END_ID: ', @v_end_id)
	);
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from COMPCONTACTMSTR
	where STE_MIGRATIONID is not null 
		AND STE_MIGRATIONSOURCE = 'DIR_SUBC' AND ste_contacttype = 'CONTACT';
	...
	values (
		@PackageName
		, @PackageLogID
		, 'COMPCONTACTMSTR-DIR_SUBC (CONTACT)'
		, 'COMPLETED'
		, CONCAT('COUNT: ', COALESCE(@v_cnt,0), ', START_ID: ', @v_start_id, ', END_ID: ', @v_end_id)
	);
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from COMPCONTACTMSTR
	where STE_MIGRATIONID is not null 
		AND STE_MIGRATIONSOURCE = 'DIR_SUBC' AND ste_contacttype = 'TECH';
