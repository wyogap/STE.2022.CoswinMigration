...
BEGIN
	...
	where STE_MIGRATIONID is not null and ldownertable='COMPMASTER' and STE_MIGRATIONSOURCE='HO_ADDR';
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='COMPMASTER' and STE_MIGRATIONSOURCE='MAIL_ADDR';
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='COMPMASTER' and STE_MIGRATIONSOURCE='RMK';
