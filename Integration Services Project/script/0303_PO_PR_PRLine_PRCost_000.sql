...
-- Add custom columns
-- ------------------
IF COLUMNPROPERTY(OBJECT_ID('dbo.pr'), 'STE_MIGRATIONID', 'ColumnId') is null
ALTER TABLE [pr]
...
drop procedure if exists ste_0303_po_pr_pre
GO

CREATE PROCEDURE ste_0303_po_pr_pre 
	@PackageLogID INT
AS
BEGIN
	-- truncate existing data
	delete from dbo.[pr] where STE_MIGRATIONID is not null
...
-- Add custom columns
-- ------------------
IF COLUMNPROPERTY(OBJECT_ID('dbo.prline'), 'STE_MIGRATIONID', 'ColumnId') is null
ALTER TABLE prline
ADD STE_MIGRATIONID bigint default null,
    STE_MIGRATIONDATE datetime NOT NULL DEFAULT (GETDATE());
...
drop procedure if exists ste_0303_po_prline_pre
GO

CREATE PROCEDURE ste_0303_po_prline_pre 
	@PackageLogID INT
AS
BEGIN
	-- truncate existing data
	delete from dbo.prline where STE_MIGRATIONID is not null;
