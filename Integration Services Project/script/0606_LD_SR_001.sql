...
drop procedure if exists ste_0606_LD_SR_pre
GO

CREATE PROCEDURE ste_0606_LD_SR_pre 
	@PackageLogID INT
AS
BEGIN
	-- truncate existing data
	delete from longdescription where STE_MIGRATIONID is not null and ldownertable='TICKET' AND STE_MIGRATIONSOURCE='SR';
...
drop procedure if exists ste_0606_LD_SR_post
GO

CREATE PROCEDURE ste_0606_LD_SR_post
  @PackageLogID INT
AS
BEGIN
	...
	where STE_MIGRATIONID is not null and ldownertable='TICKET' and ldownercol='DESCRIPTION'
		AND STE_MIGRATIONSOURCE='SR';
	...
	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='TICKET' and ldownercol='TRANSFER_REASON'
		AND STE_MIGRATIONSOURCE='SR';
	...
	values (
		@PackageName
		, @PackageLogID
		, 'TRANSFERREASON-MATCH'
		, 'COMPLETED'
		, CONCAT('COUNT: ', @v_cnt, ', START_ID: ', @v_start_id, ', END_ID: ', @v_end_id)
	);

	select @v_start_id=min(STE_MIGRATIONID), @v_end_id=max(STE_MIGRATIONID), @v_cnt=count(STE_MIGRATIONID) from longdescription
	where STE_MIGRATIONID is not null and ldownertable='TICKET'
		AND STE_MIGRATIONSOURCE='SR';
